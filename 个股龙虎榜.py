...
def parseUslAbnormalData(data):
    ...
    #print('parseUslAbnormalData %s ' % temp[['code', 'day', 'direction', 'rank']])
    return temp
...
billboard = get_billboard_list(stock_list=stock, start_date=startDate, end_date=endDate)
#营业部关联表
...
for index, row in rlatCount.iterrows():
    ...
    df = pro.daily(ts_code=code, start_date='20181101', end_date=endDate.replace('-', ''))
    df = df.iloc[::-1]   #按行倒置顺序
